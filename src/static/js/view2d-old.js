(function(global, factory) {
    ...
    const View2d = {
        ...
        color: [ '#409EFF','#ffffff','#c23531', '#E6A23C', '#67C23A', '#91c7ae', '#bda29a', '#546570'],
        baseLine: {
            color: 'orange',
            lineWidth: 1,
            space: 5,
    ...
    function save(fileName) {
        ...
        fileName = fileName || 'View2d_data_'+ Math.round(Math.random()*Math.pow(10, 8))
        let data = {
    ...
    View2d.scene = function(parentElement, option = {}) {
        ...
        View2d.$el.style.background = `${View2d.background.color} url(${View2d.background.image}) no-repeat center`
        View2d.$el.style.backgroundSize = `100% 100%`
        ...
        this.resize = function() {
            ...
            // if(View2d.mode.type == 'preview'){
                ...
                let scaleX = rectW/(_this.width - 10) / _this.scaleVal.x
                let scaleY = rectH/(_this.height - 10) / _this.scaleVal.y
                let position = {
                ...
                if(View2d.mode.type == 'preview'){
                    ...
                    _this.scale(_this.position, 1/scale,1/scale)
                }
            // }
        }
        // 场景更新
        this.update = function() {
            ...
            if(View2d.baseLine.x.length > 0) drawLine(View2d.baseLine.x);
            if(View2d.baseLine.y.length > 0) drawLine(View2d.baseLine.y);
        ...
        if(Object.keys(option).length > 0) {
            ...
            // let g = View2d.mode.type == 'preview' ? new View2d.Group() : this
            let g = this
            View2d.activeNode = null
    ...
    function drawNode(node, boolean) {
        ...
        //     ctx.scale(1/allScale.x,1/allScale.y)
        // }else{
        //     ctx.scale(1/(allScale.x - View2d.mode.previewGroup.scaleVal.x),1/(allScale.x - View2d.mode.previewGroup.scaleVal.y))
        // }
        autoText(node, scene)
        ctx.restore() 
    }
    function parentTransform() {
        if(this.parentId !== undefined) {
            let parentNode = View2d.nodes.find( item => item.groupId == this.parentId)
    ...
    function darwLinePath() {
        ...
        if(View2d.pen.showPath || node.shape == 'waterCapacity') {
            ctx.save()
            ctx.beginPath()
    ...
    View2d.Group = function(data={}) {
        if(data.vertex !== undefined) {
            ...
            this.name = 'group' + Math.floor(Math.random()*Math.pow(10,5))
            this.groupId = View2d.nodes.length + 1
            View2d.nodes.push(this)
        ...
        this.DValue = {x: 0, y: 0}
        this.name = data.name || 'group' + Math.floor(Math.random()*Math.pow(10,5))
    ...
    View2d.Group.prototype.add = function(){
        ...
        for (let i in arguments) {
            scene.children.forEach( child => {
                if(child.name === arguments[i].name) {
                    scene.remove(child)
                    if(child.shape=="drawEcharts") {
                        View2d.$dom.appendChild(child.element)
                    }
            ...
            arguments[i].parentId = this.groupId
            /** 子节点Node的所有顶点 
             * 组和到数组
    ...
    View2d.Group.prototype.remove = function(){
        ...
        this.children.forEach( (child, i) => {
            ...
            for(let k in arguments) {
                let id = arguments[k].constructor === View2d.Node ? 'nodeId' : 'groupId' 
    ...
    **/
    ...
    View2d.Node = function(data) {
        ...
        if(data.vertex !== undefined) {
            ...
            this.helperBox = false
            this.name = 'node' + Math.floor(Math.random()*Math.pow(10,5))
            this.nodeId = View2d.nodes.length
            View2d.nodes.push(this)
        ...
        this.position = {...data.position}
        this.name = data.name || 'node' + Math.floor(Math.random()*Math.pow(10,5))
    ...
    function textDefault(text = {}) {
        ...
        defaultVal.fontFamily = text.fontFamily || 'kaiTi'
        defaultVal.lineHeight = text.lineHeight || defaultVal.fontSize
    ...
    function accountVertex() {
        View2d.nodes.push(this) //保存node节点
        let xMax = 0,yMax = 0,xMin = 9999,yMin = 9999;
        for(let i = -100; i<View2d.$el.width; i+= 1) {
            for(let j = -100; j<View2d.$el.height; j+= 1) {
    ...
    * */
    function scale(scaleX, scaleY) {
        ...
        if(this.constructor == View2d.Group) {
            this.traverse( child => {
                let cX = accountNodeScale(child,scale_x,scale_y)
                setVertexVal(cX, child)
                let parentNode = View2d.nodes.find( item => item.groupId == child.parentId)
      ...
      rotateTimer = setTimeout(() => {
        ...
        if(this.constructor == View2d.Group) {
            this.traverse( child => {
                ...
                let xMin = Math.min(...xArr),xMax = Math.max(...xArr),yMin = Math.min(...yArr),yMax = Math.max(...yArr)
                child.position = {
                    x: xMin + (xMax - xMin)/2 + child.DValue.x*View2d.activeNode.scaleVal.x,
                    y: yMin + (yMax - yMin)/2 + child.DValue.y*View2d.activeNode.scaleVal.y
                }
    ...
    function setToNode(cloneNode) {
        cloneNode.children.forEach( (child, i) => {
            child.parentId = cloneNode.groupId
    ...
    * **/
    function offEvents(events) {
        ...
        eventsList = eventsList.filter( item =>{ 
            if(events === 'drag') {
                return this.nodeId !== item.node.nodeId && !(this.nodeId == item.node.nodeId && item.isDrag)
            }else{
                return !(this.nodeId == item.node.nodeId && item.events === events)
            }
        })
    ...
    * **/
    function offAllEvents() {
        this.events = []
        eventsList = eventsList.filter( item => this.name !== item.node.name)
        let temp = []
    ...
    function setEvents(events,callBack, callBack1, callBack2) {
        ...
        eventsList = eventsList.filter( item => {
            return item.events !== events || (item.events == events && item.node.name !== this.name)
        })//去除同一节点 同一事件重复保存
        if(events !== 'hover' && events !== 'drag') {
            if(isDrag) {
            ...
            if(events == 'drag') {
                ...
                this.on('mouseup',() => {
                    if(this.isMousedown) {
                        ...
                        if(typeof callBack2 == 'function') callBack2.call(this,e)
    ...
    function eventHandle(list, event) {
        ...
        let currentEventList = list.filter( item => temp.includes(item.node.name))
        ...
        if(View2d.mode.type !== "preview") {
            ...
            if(event.type === 'mousedown' && hoverVertex == -1) {
                if(trigger.length > 0) {
                    if(event.ctrlKey && View2d.activeNode !== null && View2d.activeNode.name !== trigger[0].name) {//同时按下ctrl键选择节点时-临时组合
        ...
        currentEventList.forEach( ele => {
            let scene = View2d.nodes.find( item => item.constructor === View2d.scene)
            if(!View2d.nodes.some(item => item.name === ele.node.name)) return false
            if(ele.node.checkPointInPath(x,y) && !hadTrigger) {
                hadTrigger = true
                if(ele.events !== 'drag') {
    ...
    function findVertex(x, y) {
        ...
        View2d.nodes.forEach( item => {
            if(item.zIndex !== null && View2d.activeNode.nodeId !== item.nodeId) {
    ...
    function controler(x,y,trigger) {
        ...
        if(hoverVertex == -1 && !mousedownVertex){
            ...
            if(trigger.length > 0 && !trigger[0].freeze && trigger[0].dragEvents.map(ele => ele.events).includes('drag')) {
                View2d.$el.style.cursor = 'move'
            }
        }
        if(View2d.activeNode !== null && !View2d.activeNode.freeze) {
            ...
            if(mousedownVertex) {
                ...
                switch(hoverVertex) {
                    ...
                    case 0:
                    {
                        ...
                        let scale_x = xScaleVal/View2d.activeNode.scaleVal.x
                        let scale_y = yScaleVal/View2d.activeNode.scaleVal.y
                        let pX = accountNodeScale(View2d.activeNode, scale_x, scale_y)
                        vertex[0].x += 2*pX.dx1 - 2*pX.dx;
                        vertex[0].y -= 2*pX.dy1 + 2*pX.dy;
                        vertex[1].x += 2*pX.dx1;
                        vertex[1].y -= 2*pX.dy1;
                        vertex[3].x -= 2*pX.dx;
                        vertex[3].y -= 2*pX.dy;
                        ...
                        // // // 改变子顶点位置
                        if(View2d.activeNode.constructor == View2d.Group) {
                            View2d.activeNode.traverse( child => {
                                let cX = accountNodeScale(child, scale_x, scale_y)
                                child.vertex[0].x += 2*cX.dx1 - 2*cX.dx;
                                child.vertex[0].y -= 2*cX.dy1 + 2*cX.dy;
                                child.vertex[1].x += 2*cX.dx1;
                                child.vertex[1].y -= 2*cX.dy1;
                                child.vertex[3].x -= 2*cX.dx;
                                child.vertex[3].y -= 2*cX.dy;
                                resizeEchart(child)
                                let parentNode = View2d.nodes.find( item => item.groupId == child.parentId)
                                let position = getRotatePos(child.groupPosition, {x: 0, y: 0}, -cX.rotateVal)
                                position.x *= scale_x
                                position.y *= scale_y
                                child.groupPosition = getRotatePos(position, {x: 0, y: 0}, cX.rotateVal)
                                child.translate({
                                    x: parentNode.position.x + child.groupPosition.x,
                                    y: parentNode.position.y + child.groupPosition.y
                                })

                                let xArr = [],yArr = []
                                child.vertex.forEach((item, i) => {
                                    xArr.push(child.vertex[i].x)
                                    yArr.push(child.vertex[i].y)
                                })
                                let xMin = Math.min(...xArr),xMax = Math.max(...xArr),yMin = Math.min(...yArr),yMax = Math.max(...yArr)
                                child.position = {
                                    x: xMin + (xMax - xMin)/2 + child.DValue.x*View2d.activeNode.scaleVal.x,
                        ...
                        View2d.activeNode.scaleVal.x = xScaleVal
                        View2d.activeNode.scaleVal.y = yScaleVal
                    }
                        break;
                    case 1:
                        {
                            ...
                            let scale_y = yScaleVal/View2d.activeNode.scaleVal.y
                            let pX = accountNodeScale(View2d.activeNode, scale_x, scale_y)
                            vertex[1].x += 2*pX.dx1 + 2*pX.dx;
                            vertex[1].y -= 2*pX.dy1 - 2*pX.dy;
                            vertex[2].x += 2*pX.dx;
                            vertex[2].y += 2*pX.dy;
                            vertex[0].x += 2*pX.dx1;
                            vertex[0].y -= 2*pX.dy1;
                            ...
                            if(View2d.activeNode.constructor == View2d.Group) {
                                View2d.activeNode.traverse( child => {
                                    let cX = accountNodeScale(child, scale_x, scale_y)
                                    child.vertex[1].x += 2*cX.dx1 + 2*cX.dx;
                                    child.vertex[1].y -= 2*cX.dy1 - 2*cX.dy;
                                    child.vertex[2].x += 2*cX.dx;
                                    child.vertex[2].y += 2*cX.dy;
                                    child.vertex[0].x += 2*cX.dx1;
                                    child.vertex[0].y -= 2*cX.dy1;
                                    resizeEchart(child)
                                    let parentNode = View2d.nodes.find( item => item.groupId == child.parentId)
                                    let position = getRotatePos(child.groupPosition, {x: 0, y: 0}, -rotateVal)
                                    position.x *= scale_x
                                    position.y *= scale_y
                                    child.groupPosition = getRotatePos(position, {x: 0, y: 0}, rotateVal)
                                    child.translate({x: parentNode.position.x + child.groupPosition.x, y: parentNode.position.y + child.groupPosition.y })
                    ...
                    case 2: 
                       {
                            ...
                            let scale_y = yScaleVal/View2d.activeNode.scaleVal.y
                            let pX = accountNodeScale(View2d.activeNode, scale_x, scale_y)
                            vertex[2].x -= 2*pX.dx1 - 2*pX.dx;
                            vertex[2].y += 2*pX.dy1 + 2*pX.dy;
                            vertex[1].x += 2*pX.dx;
                            vertex[1].y += 2*pX.dy;
                            vertex[3].x -= 2*pX.dx1;
                            vertex[3].y += 2*pX.dy1;
                            ...
                            if(View2d.activeNode.constructor == View2d.Group) {
                                View2d.activeNode.traverse( child => {
                                    let cX = accountNodeScale(child, scale_x, scale_y)
                                    child.vertex[2].x -= 2*cX.dx1 - 2*cX.dx;
                                    child.vertex[2].y += 2*cX.dy1 + 2*cX.dy;
                                    child.vertex[1].x += 2*cX.dx;
                                    child.vertex[1].y += 2*cX.dy;
                                    child.vertex[3].x -= 2*cX.dx1;
                                    child.vertex[3].y += 2*cX.dy1;
                                    resizeEchart(child)
                                    let parentNode = View2d.nodes.find( item => item.groupId == child.parentId)
                                    let position = getRotatePos(child.groupPosition, {x: 0, y: 0}, -rotateVal)
                                    position.x *= scale_x
                    ...
                    case 3:
                        {
                            ...
                            let scale_y = yScaleVal/View2d.activeNode.scaleVal.y
                            let pX = accountNodeScale(View2d.activeNode, scale_x, scale_y)
                            vertex[3].x -= 2*pX.dx1 + 2*pX.dx;
                            vertex[3].y += 2*pX.dy1 - 2*pX.dy;
                            vertex[0].x -= 2*pX.dx;
                            vertex[0].y -= 2*pX.dy;
                            vertex[2].x -= 2*pX.dx1;
                            vertex[2].y += 2*pX.dy1;
                            ...
                            if(View2d.activeNode.constructor == View2d.Group) {
                                View2d.activeNode.traverse( child => {
                                    let cX = accountNodeScale(child, scale_x, scale_y)
                                    child.vertex[3].x -= 2*cX.dx1 + 2*cX.dx;
                                    child.vertex[3].y += 2*cX.dy1 - 2*cX.dy;
                                    child.vertex[0].x -= 2*cX.dx;
                                    child.vertex[0].y -= 2*cX.dy;
                                    child.vertex[2].x -= 2*cX.dx1;
                                    child.vertex[2].y += 2*cX.dy1;
                                    resizeEchart(child)
                                    let parentNode = View2d.nodes.find( item => item.groupId == child.parentId)
                                    let position = getRotatePos(child.groupPosition, {x: 0, y: 0}, -rotateVal)
                                    position.x *= scale_x
                    ...
                    case 0.5: 
                    {
                        ...
                        let scale_y = yScaleVal/View2d.activeNode.scaleVal.y
                        let pX = accountNodeScale(View2d.activeNode, 1, scale_y)
                        vertex[0].x += 2*pX.dx1;
                        vertex[0].y -= 2*pX.dy1;
                        vertex[1].x += 2*pX.dx1;
                        vertex[1].y -= 2*pX.dy1;
                        ...
                        if(View2d.activeNode.constructor == View2d.Group) {
                            View2d.activeNode.traverse( child => {
                                let cX = accountNodeScale(child, 1, scale_y)
                                child.vertex[0].x += 2*cX.dx1;
                                child.vertex[0].y -= 2*cX.dy1;
                                child.vertex[1].x += 2*cX.dx1;
                                child.vertex[1].y -= 2*cX.dy1;
                    ...
                    case 1.5: 
                       {
                            ...
                            let scale_x = xScaleVal/View2d.activeNode.scaleVal.x
                            let pX = accountNodeScale(View2d.activeNode, scale_x, 1)
                            vertex[1].x += 2*pX.dx;
                            vertex[1].y += 2*pX.dy;
                            vertex[2].x += 2*pX.dx;
                            vertex[2].y += 2*pX.dy;
                            ...
                            if(View2d.activeNode.constructor == View2d.Group) {
                                View2d.activeNode.traverse( child => {
                                    let cX = accountNodeScale(child, scale_x, 1)
                                    child.vertex[1].x += 2*cX.dx;
                                    child.vertex[1].y += 2*cX.dy;
                                    child.vertex[2].x += 2*cX.dx;
                                    child.vertex[2].y += 2*cX.dy;
                ...
                if(hoverVertex !== 4) {
                    View2d.activeNode.vertex = [...vertex]
                    let center = {
                        x: vertex[0].x + (vertex[2].x - vertex[0].x)/2,
                        y: vertex[0].y + (vertex[2].y - vertex[0].y)/2
                    }
                    View2d.activeNode.position =  {
                        x: center.x + View2d.activeNode.DValue.x*View2d.activeNode.scaleVal.x, 
                        y: center.y + View2d.activeNode.DValue.y*View2d.activeNode.scaleVal.y
                    }
    ...
    function scrollScale () {
        ...
        if (delta){
            x += delta*0.03
            y += delta*0.03
            if ( x > View2d.mode.zoom.min && x < View2d.mode.zoom.max){//向下滚动 缩小
    ...
    function fivePointedStart(data, add) {//五角星
        ...
        for(var i=0;i<5;i++){
            ctx.lineTo(
                Math.cos((18+i*72)/180*Math.PI)*data.params.size,
                -Math.sin((18+i*72)/180*Math.PI)*data.params.size
            );
            ctx.lineTo(
                Math.cos((54+i*72)/180*Math.PI)*data.params.size/2.5,
                -Math.sin((54+i*72)/180*Math.PI)*data.params.size/2.5
            );
        }
        ctx.closePath()
        if(add) ctx[data.drawType]()
    ...
    function drawSwitch (data, add) {
        ...
        let color = nodeSetColor(data)
        ctx.beginPath()
        ctx.fillStyle = data.isOn ? color[0] : '#ccc'
        ctx.lineTo(-24,20)
        ctx.arcTo( -150,20, -24,-20,20)
        ctx.lineTo(-24,-20)
        ctx.lineTo(24,-20)  
        ctx.arcTo( 150 ,0, 24,20,20)
        ctx.lineTo(24,20)
        ctx.lineTo(-24,20)
        if(add) {
            ctx.fill()
        ...
        ctx.beginPath()
        ctx.fillStyle = color[1] || '#fff'
        let offsetX = data.isOn? 20:-20
        ctx.arc(offsetX,0,18,0,2*Math.PI)
        if(add)ctx.fill()
        ctx.beginPath()
    ...
    function windmill(data, add) {
        ...
        if(add) ctx[data.drawType]()
        ctx.beginPath()
        ctx.rect(-data.params.w, -data.params.w, data.params.w*2, data.params.h + data.params.w)
        if(add && data.border.show) {
            ctx.lineWidth = data.border.lineWidth || 2
            ctx.strokeStyle = data.border.color || 'red'
            ctx.lineCap = data.border.lineCap || 'butt'
    ...
    function bezierLine(data, add, pen) {
        ...
        let color = nodeSetColor(data)
        ctx.fillStyle = color[1]
        data.linePath.forEach( (item, i) => {
            let x = item.x
            let y = item.y
            let type = item.type || 'curve'
        ...
        ctx.strokeStyle = color[0]
        _darwlinePath(points, ctx)
        if(add) ctx[data.drawType]()
    }
    function _darwlinePath(points,ctx) {
